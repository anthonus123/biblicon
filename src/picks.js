// Pericope -> the Orthodox icons shown for it.
//
// ONE SCENE, MANY ICONS; ONE ICON, ONE PASSAGE. A passage may list several files, and every
// one of them depicts THAT passage's own scene — the Theophany at Ohrid beside the Theophany
// at Langadas beside the Theophany in the Hermitage. The Church has painted these scenes many
// times, and seeing three hands paint the same moment is the point of the gallery.
//
// What a second icon may NOT be: a stand-in. It has to be the same scene as the first, and it
// may not change the passage's tier — a Pantokrator does not go beside a Theophany, and an
// Anastasis does not go beside the myrrhbearers at the tomb, because that is the Church's icon
// OF the Resurrection rather than a picture of Matthew's verses. A passage with no icon of its
// own still gets none: it renders as a plain verse row, tier c.
//
// The first file listed is the primary. It leads the card, and it is the one that carries the
// positioned "Deciphering the Icon" markers; the others each carry their own prose reading.
// No file appears under two passages, and `make check` fails if one does.
//
// Every placement below was checked against the image itself, not its title — see HANDOFF
// Gotchas. Passages absent from this map are deliberate: Orthodox iconography is built on the
// feast cycle, the miracles and the saints, and has no scene-icon for most of the parables and
// teaching passages.
module.exports={
  "genealogy": [
